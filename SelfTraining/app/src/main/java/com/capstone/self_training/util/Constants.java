package com.capstone.self_training.util;

public class Constants {
    public static final String port = ":8080/";
    public static final String domain = "192.168.12.101";
    public static final String header_string = "Authorization";
    public static final int Status_Ok = 200;
    public static final int Status_Conflit = 409;
    public static final int Status_Created = 201;
    public static final int Status_Forbidden = 403;

    public static final String GENDER_MALE = "male";
    public static final String GENDER_FEMALE = "female";

    public static final String USER_IMAGE_FOLDER_UPLOAD_FIREBASE = "User image";
    public static final String COURSE_THUMBNAIL_FIREBASE_FOLDER = "Course thumbnail";

    //status
    public static final String STATUS_ACTIVE = "active";
    public static final String SUB_STATUS_ACTIVE = "Everyone";

    public static final String STATUS_UNACTIVE = "unactive";
    public static final String SUB_STATUS_UNACTIVE = "Just me";

}
